...
The smallest v1 environment is a taskset package. It imports `verifiers.v1`, defines a typed `Task`, defines a typed `TasksetConfig`, subclasses `Taskset`, and exports that class through `__all__`.

`environments/reverse_text/reverse_text.py` is the canonical tiny example:
...
- Config fields are runner-tunable knobs exposed through `[taskset]` or `--taskset.*`.
- Rewards read `Trace`; they do not parse framework internals.
