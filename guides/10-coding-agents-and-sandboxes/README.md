...
This is the smallest useful sandbox pattern: one task, one Python tool, one isolated runtime, and a deterministic reward. The Python tool starts an IPython kernel inside a plain `python:3.11-slim` sandbox and installs the packages it needs during sandbox setup, so the example shows exactly what is available at execution time.

In the implementation, the Python tool is a Taskset method. `MathPythonTaskset.load_toolsets(config)` exposes it with a sandboxed `vf.Toolset`; `@vf.cleanup` and `@vf.reward` live on the same Taskset because they are part of the same task logic.
