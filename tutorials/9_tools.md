...
Some tasks are unanswerable from parametric memory — the model has to *look things up*. In this tutorial you will study `wiki-search`, an environment where the model answers trivia questions by searching a Wikipedia corpus through custom tools, and learn how v1 packages tools as `vf.Toolset` MCP servers.
...
- **`setup()` runs once per server**, before any rollout uses it. Expensive, task-agnostic work belongs here — this one loads the corpus and builds an embedding index. There is also `setup_task(task)` for per-task preparation ([Coding Agent Environments](11_coding_agents.md) uses it).
...
Scoring is a judge reward ([Judges](6_judges.md)) comparing the model's final answer against the trivia ground truth — tools change *how* the model works, not how it is scored. The tool calls themselves land in `trace.tool_messages`, so you can audit search behavior in the traces.
...
- **`shared = true`** — one server for all rollouts; right for expensive read-mostly setup like this Chroma index. Per-rollout mutable state must then live in `self.state` (a typed `vf.State`, serialized through the state channel), never on `self`, or rollouts will trample each other.
...
This environment trains as-is — `configs/08/wiki-search-rl.toml` embeds it in a training config exactly as [tutorial 3](3_first_rl.md) described, tools included. The [Search Agent](17_search_agent.md) recipe walks that full loop.
...
→ [Multimodal Environments](10_multimodal.md): tasks that carry images, not just text.
