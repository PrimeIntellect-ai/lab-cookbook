...
This series of tutorials is meant to get you up and running on Prime Intellect Lab, from an empty terminal to evaluating frontier models, training models with reinforcement learning, and automatically optimizing prompts. We'll be starting gently, so no prior experience with evals or RL is assumed.
...
You will meet these components across the tutorials. Each one of the pieces is introduced properly when you first use it, but let's go through a quick overview.

Your front door to the platform is the **`prime` CLI**. You can use it to run all the platform functionality from your terminal. For example, `prime train` launches training, `prime gepa run` optimizes prompts, `prime env init` scaffolds new environments. (Evals themselves run through the `verifiers` eval runner, `uv run eval` — you'll meet it in tutorial 2.) We will be installing the CLI in the next tutorial.
...
![The pieces: verifiers environments, the Hub, evals, training, and sandboxes](the-pieces.svg)
