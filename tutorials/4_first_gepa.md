...
You've probably experienced prompt engineering by hand, where you've tweaked a system prompt, eyeballed a few responses, and repeated the procedure. This is a slow and unsystematic procedure, and it's easy to fool yourself with three cherry-picked examples.

`verifiers` enfironments already comes with the ability to score rollouts, so that means that the effectiveness of different prompts can be compared, and therefore we can search for optimal prompts. Prompt optimization is just a search over prompts, with your environment as the judge. The only missing ingredient is something to *propose* better prompts than you'd think of yourself. This is what GEPA (Genetic-Pareto prompt evolution) does -- it automates the loop you'd do by hand, with two clever twists:

1. **Evaluate** the current prompt on a small batch of tasks.
2. **Reflect**, where a second LLM (the *reflection model*) reads the actual failed rollouts — the full transcripts — and writes a diagnosis: *"the model keeps repeating letters it already ruled out; the instructions never tell it to track them."*.
...
5. Repeat until the evaluation budget runs out; the winner is validated on held-out tasks.
...
We'll optimize a prompt for Wordle, the word game from Tutorial 2. It's a great subject because strategy lives in the prompt: a model told merely "play Wordle" plays sloppily, while a prompt that says how to track confirmed letters, eliminate impossible ones, and choose information-rich guesses plays measurably better. There is real reward to be earned with words alone.

## Run it

```bash
prime gepa run primeintellect/wordle \
  --model openai/gpt-5.4-mini \
  --reflection-model openai/gpt-5.4 \
  --num-train 100 --num-val 50 \
  --max-calls 500
```
...
| Flag | Meaning |
| --- | --- |
| `primeintellect/wordle` | The environment whose reward we're optimizing against. |
...
| `--max-calls 500` | **Your budget.** GEPA's currency is scored rollouts; this caps the total at 500, so cost is bounded and known before you start. More budget = more evolution steps = usually better prompts, with diminishing returns. |

> **A note on plumbing:** the GEPA CLI currently drives environments through the legacy (v0) loader — which is why the environment is given as a Hub id like `primeintellect/wordle` and why its knobs are passed as `--env-args` JSON rather than the `--taskset.*` flags you saw in evals. A config-file form (TOML with an `[gepa]` table) exists too — see `configs/04/wordle-gepa.toml` in this repo for the shape.
...
The run saves everything under an outputs directory (printed at the end, of the form `outputs/gepa/<env>--<model>/<run-id>/`): the best prompt found, the full lineage of candidates with their scores, and the validation results.
...
- Raise `--max-calls` to 1000 on a cheap player model. Where do the gains flatten?
