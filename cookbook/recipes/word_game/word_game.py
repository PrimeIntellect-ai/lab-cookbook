...
class WordleEnv(vf.MultiTurnEnv):
    ...
    async def env_response(self, messages: list[dict], state: dict, **kwargs) -> list[dict]:
        ...
        if guess is None:
            ...
            return [{"role": "user", "content": "Invalid guess. Please reply with exactly one 5-letter word."}]
        ...
        if guess == state["secret"]:
            state["solved"] = True
            final_msg = [{"role": "user", "content": f"Feedback: {feedback}\nCongratulations! You guessed correctly!"}]
    ...
    @vf.stop
    async def stop_on_solved(self, state: dict, **kwargs) -> bool:
        return state.get("solved", False)

    @vf.stop
    async def stop_on_max_guesses(self, state: dict, **kwargs) -> bool:
    ...
    @vf.cleanup
    async def cleanup_game(self, state: dict) -> None:
