...
class MathPythonTaskset(vf.Taskset[MathPythonTasksetConfig]):
    ...
    sandbox_root: ClassVar[str] = "/tmp/vf_math_python"
    executor_path: ClassVar[str] = "/tmp/vf_math_python/execute_cell.py"
...
for _ in $(seq 1 {self.config.kernel_start_timeout_seconds * 10}); do
...
"""
        result = await sandbox.execute(
            command,
            timeout=self.config.kernel_start_timeout_seconds,
        )
        if result.exit_code:
    ...
    def load_tasks(self, split: vf.TaskSplit = "train") -> vf.Tasks:
        source_split = "test" if split == "eval" else "train"
