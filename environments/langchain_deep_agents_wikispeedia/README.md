...
- **Scoring**: binary `reached_target` reward plus zero-weight path/tool metrics. `path_efficiency_reward` contributes `efficiency_weight * path_efficiency`.
...
prime eval run langchain-deep-agents-wikispeedia \
  ...
  -a '{"config": {"taskset": {"min_path_length": 4, "max_path_length": 6}, "harness": {"max_turns": 40}}}'
```
...
| `efficiency_weight` | float | `0.0` | If `> 0`, add `efficiency_weight * path_efficiency` to the reward (a near-optimal route earns up to `1 + efficiency_weight`; a wanderer that reaches the target still earns `1`). Default `0.0` keeps reward as pure binary reachability. |
...
| `path_efficiency` | `shortest_path / actual_path_length` if reached, else 0. Zero-weight diagnostic. |
| `path_efficiency_reward` | Weighted contribution: `efficiency_weight * path_efficiency`. |
...
### Notes
- This package is an advanced adapter example, not the default cookbook shape.
  Most new environments should attach taskset/harness-specific logic directly to
  the owning Taskset or Harness. The extra module-level graph, tool, and metric
  plumbing here exists to bridge SNAP Wikispeedia and LangChain Deep Agents.
- With default config, reward is `reached_target` only — exact, deterministic, no judge required. The deep-agent structural metrics are zero-weight so they show up in eval tables without shaping the policy.
